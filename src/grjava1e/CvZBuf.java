...
public void paint(Graphics g) {
      ...
      for (int j = 0; j < nFaces; j++) {
         ...
         // obj.colorCodeLambert(pol.getA(), pol.getB(), pol.getC());
         // obj.colorCode(pol.getA(), pol.getB(), pol.getC());
         g.setColor(new Color(cCode)); // (cCode[0], cCode[1], cCode[2]));
         ...
         for (int i = 0; i < t.length; i++) {
            ...
            double u1 = b.x - a.x, v1 = c.x - a.x, u2 = b.y - a.y,
                  v2 = c.y - a.y, cc = u1 * v2 - u2 * v1;
            if (cc <= 0)
               continue;
            double xA = a.x, yA = a.y, xB = b.x, yB = b.y, xC = c.x,
                  yC = c.y, xD = (xA + xB + xC) / 3,
                  yD = (yA + yB + yC) / 3, zDi = (zAi + zBi + zCi) / 3,
            ...
            for (int y = yBottom; y <= yTop; y++) { // Compute horizontal line
                                                    // segment (xL, xR)
                                                    // for coordinate y:
               ...
               /*
                * for (int x=iXL; x<=iXR; x++) { if (zi < buf[x][iy]) // < is
                * nearer { g.drawLine(x, iy, x, iy); buf[x][iy] = (float)zi; }
                * zi += dzdx; }
                */
